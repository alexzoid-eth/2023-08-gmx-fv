...
if [ -z "$CONTRACTS_DIR" ] || [ -z "$CONFIG_NAME" ]; then
    echo "usage:"
    echo "  ./createMutation.sh [CONTRACTS_DIR] [CONFIG_NAME] : generates a patch containing a manually injected bug and then restores the contracts directory"
    echo "Example:"
    echo "  ./createMutation.sh ../contracts/rewards oracle"
    exit 0
fi
